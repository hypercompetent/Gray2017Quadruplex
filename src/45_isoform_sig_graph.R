...
#output directory
outdir <- "isoform_sig_plot/"

#gene sets
gene_set_file <- "../geo/GSE60630_PDC3_vs_DMSO_splicing.diff"
gene_sets <- read.table(gene_set_file,stringsAsFactors=F, header = T)
...
#group definitions
groups <- list(gene_sets$q_value < 0.05,
               gene_sets$q_value >= 0.05)
group_colors <- c("purple","black")
group_names <- c("isoform_sig","isoform_nc")
...
#refseq filtering
filter_file <- "../geo/GSE60630_PDC3_vs_DMSO_splicing.diff"
filter <- read.table(filter_file,sep="\t",stringsAsFactors=F,header = T)
filter <- filter[filter$status == "OK",]
names(filter)[2] <- "name"
...
for(reg in regions) {
  ...
  plotname <- paste(outdir,reg,"_mp_plot.pdf",sep="")
  region_plot
  ggsave(plotname,width=8.57,height=6)
...
for(reg in regions) {
  ...
  plotname <- paste(outdir,reg,"_p_plot.pdf",sep="")
  region_plot
